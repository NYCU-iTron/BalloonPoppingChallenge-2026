...
class Vehicle:
    ...
    def __init__(self, given_parameters):
        ...
        self.mass_flow_rate = float(
            tank[Schema.Given.Tank.LIQUID_MASS_FLOW_RATE_OUT]
            + grain_mass / self.burn_time
        )
        ...
        nozzle_position = float(
            motor[Schema.Given.Motor.MOTOR_POSITION]
            + motor[Schema.Given.Motor.NOZZLE_POSITION]
        )
        self.tvc_lever = abs(
            nozzle_position
            - float(body[Schema.Given.RocketBody.CENTER_OF_MASS_WITHOUT_MOTOR])
        )
        ...
        # ------------------------------- Actuators ------------------------------- #
        self.max_gimbal = float(control[Schema.Given.Control.MAX_GIMBAL_ANGLE])
    ...
    def mass(self, t: float) -> float:
        ...
        return self.initial_mass - self.mass_flow_rate * min(
            max(t, 0.0), self.burn_time
        )
    ...
    def min_climb_inclination(
        self, t: float, required_climb_accel: float = 1.0
    ) -> float:
    ...
    def transit_time(
        ...
        climb_sin : float
            ...
            -1 straight down.
        """
        travel_time, _ = self.transit_state(
            length,
            turn_angle,
            t_elapsed,
            climb_sin,
    ...
    def transit_state(
        ...
        """Estimate leg time and speed carried into the following leg.

        ``transit_time`` retains its original conservative launch-planning
        behaviour. Mid-flight planning can additionally supply the measured
        incoming speed, preventing a 20--25 m/s rocket from being silently
        reset to the nominal 15 m/s planning speed after every target.
        """
        ...
        closing_accel = float(
            np.clip(
                max_accel - GRAVITY * climb_sin,
                self.min_transit_accel,
                max(max_accel, self.min_transit_accel),
            )
        )
        ...
        entry_speed = float(
            max(
                min(carried_speed, corner_speed) * self.corner_speed_retention,
                0.0,
            )
        )

        # Acceleration rebuilds speed lost in a corner, but a smooth leg does
        # not throw away speed merely because it is above the nominal planning
        # value. This is the state the old scalar-time model could not carry.
        target_speed = max(self.planning_speed, entry_speed)
        time_to_cruise = (target_speed - entry_speed) / closing_accel
        distance_to_cruise = (
        ...
        if length <= distance_to_cruise:
            ...
            exit_speed = entry_speed + closing_accel * travel
        else:
            travel = time_to_cruise + (length - distance_to_cruise) / max(
                target_speed, 1e-9
            )
        ...
        return total_time, float(exit_speed)
