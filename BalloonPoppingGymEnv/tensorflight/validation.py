"""Holdout transfer validation against the unmodified official simulator."""
...
def _create_policy_fixture(path: Path, *, seed: int) -> dict[str, object]:
    ...
    source = build_scenario1_source(seed=seed)
    trainer = TensorFlightTrainer(
        source,
        TensorFlightTrainingConfig(
            num_envs=1,
            horizon=1,
...
def generate_closed_loop_transfer_report(
    ...
    *,
    ...
    policy_label: str = "deterministic_deployment_policy",
) -> dict[str, object]:
    ...
    if agent_factory is None:
        ...
        def agent_factory(parameters: dict[str, object]) -> BaseAgent:
            return TensorFlightAgent(parameters, artifact_path=deployment_path)
...
def generate_transfer_report(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="tensorflight_policy_") as directory:
