"""Phase 5 holdout-transfer report and decision gates."""
...
from experiments.cuda.alfonso_compat_agent import AlfonsoCompatibilityAgent
from experiments.cuda.phase5_transfer import evaluate_transfer_gates
...
def test_alfonso_fixture_uses_v011_observation_boundary_and_resets() -> None:
    _, given = load_scenario_parameters(1)
    agent = AlfonsoCompatibilityAgent(given, launch_time=1.0, target_count=1)
    elevation = float(given["environment"]["elevation"])
    observation = {
        "simulation_time": 0.0,
        "balloon_status": np.asarray([1]),
        "balloon_states": np.asarray([[10.0, 0.0, elevation + 20, 0.0, 0.0, 0.0]]),
        "rocket_sensors": np.full(12, np.nan),
    }

    assert not agent.get_action(observation)["launch"]
    observation["simulation_time"] = 1.0
    assert agent.get_action(observation)["launch"]
    agent.target_cursor = 1
    agent.reset()
    assert agent.target_cursor == 0
    assert "gimbal_range" not in inspect.getsource(AlfonsoCompatibilityAgent)


def test_alfonso_fixture_does_not_reference_oracle_state() -> None:
    source = inspect.getsource(
        __import__("experiments.cuda.alfonso_compat_agent", fromlist=["*"])
    )

    assert "rocket_states" not in source
    assert "_rocket_states" not in source
