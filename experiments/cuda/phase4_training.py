...
from experiments.cuda.phase3_balloon import BalloonBatchParameters, quaternion_to_matrix
from experiments.cuda.phase3_environment import TensorFlightEnvironment
from experiments.cuda.phase4_factory import (
    Phase4EnvironmentSource,
    build_phase4_source,
    make_phase4_environment,
)
from experiments.cuda.phase4_observation import TensorAgentAdapter
...
class Phase4Trainer:
    ...
    def __init__(
        self, source: Phase4EnvironmentSource, config: Phase4TrainingConfig
    ) -> None:
        config.validate()
        self.source = source
        self.config = config
        ...
        self.environment = make_phase4_environment(
            source,
            config.num_envs,
    ...
    def load_checkpoint(self, path: str | Path, *, strict_hashes: bool = True) -> None:
        ...
        if checkpoint.get("schema_version") != CHECKPOINT_SCHEMA:
            raise ValueError("unsupported Phase 4 checkpoint schema")
